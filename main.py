...
class MainApp(App):
    ...
    def on_start(self):
        ...
        label_data.text = datetime.now().strftime('%d/%m/%Y - %H:%M')
    ...
    def carregar_infos_usuario(self):
        try:
            ...
            requisicao = requests.get(f"https://aplicativovendashash-886bd-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}")
            ...
            pagina_ajustes.ids['id_vendedor'].text = f"[color=#000000]Seu Id Único: [b]{id_vendedor}[/b][/color]"

            # preencher total de vendas
            total_vendas = requisicao_dic['total_vendas']
            self.total_vendas = total_vendas
            homepage = self.root.ids['homepage']
            homepage.ids['label_total_vendas'].text = f"[color=#000000]Total de Vendas: [b]R$ {total_vendas}[/b][/color]"
            ...
            lista_venderores = pagina_listavendedores.ids["lista_vendedores"]

            for id_vendedor_equipe in lista_equipe:
                if id_vendedor_equipe != "":
                    banner_vendedor = BannerVendedor(id_vendedor=id_vendedor_equipe)

                    lista_venderores.add_widget(banner_vendedor)
    ...
    def mudar_foto_perfil(self, foto, *args):
        ...
        info = f'{{"avatar": "{foto}"}}'
        requisicao = requests.patch(f"https://aplicativovendashash-886bd-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}",
                                    data=info)

        self.mudar_tela("ajustespage")

    def adicionar_vendedor(self, id_vendedor_adicionado):
        link = f'https://aplicativovendashash-886bd-default-rtdb.firebaseio.com/.json?orderBy="id_vendedor"&equalTo="{id_vendedor_adicionado}"'
        ...
        if requisicao_dic == {}:
            ...
            if id_vendedor_adicionado in equipe:
                mensagem_texto.text = "Vendedor ja está na equipe"
            else:
                self.equipe = self.equipe + f",{id_vendedor_adicionado}"
                info = f'{{"equipe": "{self.equipe}"}}'
                requests.patch(f"https://aplicativovendashash-886bd-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}",
                               data=info)
                mensagem_texto.text = "Vendedor adicionado à equipe"
                pagina_listavendedores = self.root.ids["listarvendedorpage"]
                lista_venderores = pagina_listavendedores.ids["lista_vendedores"]
    ...
    def selecionar_cliente(self, foto, *args):
        ...
        # pintar de branco o texto dos demais itens
        pagina_adicionarvendas = self.root.ids["adicionarvendaspage"]
        lista_clientes = pagina_adicionarvendas.ids["lista_clientes"]
    ...
    def selecionar_unidade(self, id_label, *args):
        ...
        pagina_adicionarvendas.ids[id_label].color = (1, 215/255, 0, 1)
    ...
    def adicionar_venda(self):
        ...
        preco = pagina_adicionarvendas.ids["preco"].text
        ...
        if cliente and produto and unidade and preco and quantidade and (type(preco) == float) and (type(quantidade) == float):
            ...
            info = (f'{{"cliente": "{cliente}", "produto": "{produto}", "foto_produto": "{foto_produto}",'
                    f' "foto_cliente": "{foto_cliente}", "unidade": "{unidade}", "preco": "{preco}",'
                    f' "quantidade": "{quantidade}", "data": "{data}"}}')

            link = f"https://aplicativovendashash-886bd-default-rtdb.firebaseio.com/{self.local_id}/vendas.json?auth={self.id_token}"
            ...
            banner = BannerVenda(cliente=cliente, produto=produto, foto_cliente=foto_cliente, foto_produto=foto_produto,
                                 unidade=unidade, preco=preco, quantidade=quantidade, data=data)

            pagina_homepage = self.root.ids['homepage']
            lista_vendas = pagina_homepage.ids['lista_vendas']
            lista_vendas.add_widget(banner)

            requisicao = requests.get(f"https://aplicativovendashash-886bd-default-rtdb.firebaseio.com/{self.local_id}/total_vendas.json?auth={self.id_token}")
            total_vendas = float(requisicao.json())
            total_vendas += preco * quantidade
            info = f'{{"total_vendas": "{total_vendas:,.2f}"}}'
            requests.patch(f"https://aplicativovendashash-886bd-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}",data=info)
            homepage = self.root.ids['homepage']
            homepage.ids['label_total_vendas'].text = f"[color=#000000]Total de Vendas: [b]R$ {total_vendas}[/b][/color]"
    ...
    def carregar_todas_vendas(self):
        ...
        requisicao = requests.get(f'https://aplicativovendashash-886bd-default-rtdb.firebaseio.com/.json?orderBy="id_vendedor"')
        requisicao_dic = requisicao.json()

        # preencher foto de perfil
        foto_perfil = self.root.ids['foto_perfil']
        foto_perfil.source = f"icones/fotos_perfil/zelda.png"
        ...
        # preencher total de vendas
        pagina_todasvendas.ids['label_total_vendas'].text = f"[color=#000000]Total de Vendas: [b]R$ {total_vendas:,.2f}[/b][/color]"

        self.mudar_tela("todasvendaspage")

    def sair_tela(self ,id_tela):
    ...
    def carregar_vendas_vendedor(self, dic_info_vendedor, *args):
        ...
        pagina_vendasoutrovendedor.ids['label_total_vendas'].text = f"[color=#000000]Total de Vendas: [b]R$ {total_vendas:,.2f}[/b][/color]"
